...
(() => {
	...
	window.requestAnimationFrame = function (callback) {
		if (!warningRequestAnimationFrameDisplayed) {
			safeWarn("SingleFile is hooking the requestAnimationFrame and cancelAnimationFrame functions to load deferred images in background tabs.");
	...
	if (window.FontFace) {
		...
		window.FontFace = function () {
			if (!warningFontFaceDisplayed) {
				safeWarn("SingleFile is hooking the FontFace constructor to get font URLs.");
	...
	if (window.IntersectionObserver) {
		...
		window.IntersectionObserver = function () {
			if (!warningIntersectionObserverDisplayed) {
				safeWarn("SingleFile is hooking the IntersectionObserver API to detect and load deferred images.");
