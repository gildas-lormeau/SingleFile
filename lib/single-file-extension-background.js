(function () {
	...
	class MediaUploader {
		...
		async upload(indexFilename = 1) {
			...
			if (response.files.length) {
				if (this.filenameConflictAction == CONFLICT_ACTION_OVERWRITE$2) {
					method = "PATCH";
					fileId = response.files[0].id;
					this.metadata.parents = null;
				} else if (this.filenameConflictAction == CONFLICT_ACTION_UNIQUIFY$3) {
				...
				} else if (this.filenameConflictAction == CONFLICT_ACTION_PROMPT$2) {
					if (this.prompt) {
	...
	async function downloadCompressedContent(message, tab) {
		...
		if (skipped) {
			...
			if (message.openEditor) {
				onEdit(tabId);
				await open({ tabIndex: tab.index + 1, filename: message.filename, content: Array.from(new Uint8Array(await blob.arrayBuffer())), compressContent: true });
			} else {
				try {
					const prompt = filename => promptFilename(tabId, filename);
					let response;
	...
	async function downloadPageForeground(taskId, filename, content, tabId) {
		const serializer = getSerializer({ filename, taskId, content: await content.arrayBuffer() });
