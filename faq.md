...
By default the ZIP payload is wrapped by `<!--` and `-->` tags. However, if the payload contains the closing tag (i.e. `-->`), then the payload is wrapped in another pairs of tags (i.e. tags of `noscript` or `script` or `xmp` or `plaintext` elements) whose closing tag does not conflict with the payload. Within this HTML page, there is also a script weighting approximately 50KB designed to extract and display the ZIP payload when the file is opened in a web browser and interpreted as a web page.
